...
rule run_scibmetrics_benchmarker:
    input:
        script="scripts/run_scibmetrics_benchmarker.py",
        adata_path="outputs/{scenario}/{pipeline}_all_methods.h5ad",
    output:
        path="outputs/{scenario}/metrics/{criteria}/scib/{pipeline}_scibmetrics_benchmarker_{key}.parquet",
    params:
        eval_keys=config["eval_key"]
    log:
        "logs/{scenario}/{criteria}_{pipeline}_scibmetrics_benchmarker_{key}.log"
    conda:
    ...
    shell:
        """
        export PYTHONPATH=$(dirname $(pwd)):$(pwd) && python {input.script} {output.path} {params.eval_key} &> {log}
        """
